...
async function sendAnnouncementEmail(
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"Alzato Overseas" <${process.env.SMTP_FROM}>`,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 800px; margin: 0 auto;">
          ...
          <div style="padding: 30px; background: #f9f9f9; border: 1px solid #e0e0e0;">
            ...
            <h2 style="color: #3a3886; margin-top: 0;">
              ${title}
            </h2>
  ...
  try {
    ...
    if (!body.announcementId) {
      return NextResponse.json(
        {
          success: false,
          message: "Missing announcementId",
        },
        { status: 400 }
      );
    }
    ...
    // Fetch emails
    const emails =
      USE_TEST_EMAILS && TEST_EMAILS.length > 0
        ? TEST_EMAILS
        : await getAllUserEmails();
    ...
    return NextResponse.json({
      success: true,
      message: `Email sent successfully to ${result.sentCount} users`,
