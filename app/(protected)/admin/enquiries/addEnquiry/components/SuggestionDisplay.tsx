...
const SuggestionTable = ({ customSuggestions, exportCSV }: { customSuggestions: Program[], exportCSV: () => void }) => (
      ...
      <table className="w-full border-collapse border border-gray-300">
        <thead>
          <tr className="bg-gray-100">
            <th className="border border-gray-300 px-4 py-3 text-left text-sm font-medium text-gray-700">University</th>
            <th className="border border-gray-300 px-4 py-3 text-left text-sm font-medium text-gray-700">Program</th>
            <th className="border border-gray-300 px-4 py-3 text-left text-sm font-medium text-gray-700">Study Level</th>
            <th className="border border-gray-300 px-4 py-3 text-left text-sm font-medium text-gray-700">Duration</th>
            <th className="border border-gray-300 px-4 py-3 text-left text-sm font-medium text-gray-700">Percentage</th>
            <th className="border border-gray-300 px-4 py-3 text-left text-sm font-medium text-gray-700">IELTS</th>
          </tr>
        </thead>
        <tbody>
          {customSuggestions.map((prog) => (
            <tr key={prog.id} className="hover:bg-gray-50">
              <td className="border border-gray-300 px-4 py-3 text-sm text-gray-900">{prog.university}</td>
              <td className="border border-gray-300 px-4 py-3 text-sm text-gray-900">{prog.programme_name}</td>
              <td className="border border-gray-300 px-4 py-3 text-sm text-gray-900">{prog.study_level}</td>
              <td className="border border-gray-300 px-4 py-3 text-sm text-gray-900">{prog.duration}</td>
              <td className="border border-gray-300 px-4 py-3 text-sm text-gray-900 text-center">{prog.percentage_required || "-"}</td>
              <td className="border border-gray-300 px-4 py-3 text-sm text-gray-900 text-center">{prog.ielts_score || "-"}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
const MainSuggestions = ({ suggestions, setShowSuggestions }: { suggestions: Suggestion[], setShowSuggestions: (show: boolean) => void }) => (
    ...
    {suggestions.map((suggestion) => (
      <div key={suggestion.program.id} className="bg-white shadow rounded-lg p-6">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h3 className="text-xl font-semibold text-gray-900">{suggestion.program.programme_name}</h3>
...
export default function SuggestionDisplay({
  ...
  if (showSuggestions) {
    return <MainSuggestions suggestions={suggestions} setShowSuggestions={setShowSuggestions} />;
  }
  
  // This component will also render the custom suggestions table if showSuggestions is false
  if (customSuggestions.length > 0) {
    return <SuggestionTable customSuggestions={customSuggestions} exportCSV={exportCSV} />;
  }
