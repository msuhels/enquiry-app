...
export default function CustomAcademicSuggestion() {
  ...
  const handleSuggest = () => {
    ...
    // Filter dummy programs if field and value provided
    if (selectedField && fieldValue !== '') {
      const filtered = dummyPrograms.filter((p) => {
        const key = selectedField.toLowerCase().replace(' ', '_') as keyof Program;
        return p[key] !== undefined && p[key]! >= fieldValue;
      });
      setSuggestions(filtered);
    } else {
      setSuggestions(dummyPrograms);
    }
  };

  const exportPDF = () => {
    const doc = new jsPDF();
    const tableColumn = ['University', 'Program', 'Study Level', 'Percentage', 'IELTS'];
    const tableRows: any[] = [];
      ...
      <div className="bg-white shadow rounded-lg p-6 w-full max-w-lg space-y-4">
        ...
        <div>
          ...
          <input
            type="number"
            value={fieldValue}
            onChange={(e) => setFieldValue(e.target.value === '' ? '' : parseFloat(e.target.value))}
            className="w-full px-3 py-2 border border-gray-300 rounded-md"
        ...
        >
          <PlusIcon className="h-4 w-4 mr-2" /> Suggest
        </button>
      </div>

      {suggestions.length > 0 && (
        <div className="bg-white shadow rounded-lg p-6 w-full max-w-4xl mt-8 space-y-4">
          ...
          <table className="w-full table-auto border border-gray-300">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="border px-3 py-2">Study Level</th>
            ...
            <tbody>
              {suggestions.map((prog) => (
                <tr key={prog.id} className="text-center">
                  <td className="border px-3 py-2">{prog.university}</td>
                  <td className="border px-3 py-2">{prog.programme_name}</td>
                  <td className="border px-3 py-2">{prog.study_level}</td>
                  <td className="border px-3 py-2">{prog.percentage_required || '-'}</td>
                  <td className="border px-3 py-2">{prog.ielts_score || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
